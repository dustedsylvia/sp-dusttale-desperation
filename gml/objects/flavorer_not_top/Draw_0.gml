...
if (needs_page_init) {
	...
	//try { current_line_text = lines[current_line]; } catch(e){show_debug_message(e); exit;}
	...
	current_page += 1;
	try {
		if (array_length(portraits) >= current_page+1 and portraits[current_page] != noone) {
			currentportrait = portraits[current_page];
			text_line1_offset_x = 174;
			text_line2_offset_x = 174;
			text_line3_offset_x = 174;
			asteriskxoffset = 142;
		}
	}catch(e){}; // faces might not have anything at that index
}
...
if (text_char_index < string_length(current_line_text) and clock % textspd == 0 and pauseforframes == 0) {
	text_char_index += 1;
	audio_stop_sound(text_voicebeep);
		...
		try {
			if (pauseforframescache[i][0] == text_char_index) {
				pauseforframes = pauseforframescache[i][1];
		                   ...
		                   // fuck does that even meaOHHHHHHHHHHHHHHH
	}
...
if (clock >= 2) {
	if (current_line >= 0 and line1_asterisk) {
		draw_sprite_ext(asterisk_sprite, 0, textbox_x + asteriskxoffset, textbox_y + 32, 1, 1, 0, asteriskcolor1, 1);
	}
	if (current_line >= 1 and line2_asterisk) {
		draw_sprite_ext(asterisk_sprite, 0, textbox_x + asteriskxoffset, textbox_y + 68, 1, 1, 0, asteriskcolor2, 1);
	}
	if (current_line >= 2 and line3_asterisk) {
		draw_sprite_ext(asterisk_sprite, 0, textbox_x + asteriskxoffset, textbox_y + 104, 1, 1, 0, asteriskcolor3, 1);
	}
}
...
if (currentportrait != noone) {
	// It's important to note that the sprite must have the origin as "Middle Center".
	draw_sprite(currentportrait, 0, textbox_x + 70, (textbox_y + (textbox_y + textbox_outer_height)) / 2);
}