if (debug) {
	instance_create_depth(0, 0, -9999, time);
	audio_stop_all();
...
if (agreedToTermsOfDeathAndDespair) {
	...
	if (clock == 120) {
		music = audio_play_sound(menu, 2, true);
		repeat (7) {
	...
	if (clock >= 181 and active) {
		if (state == "main") {
			...
			if (c_pressed) {
				var fade1 = instance_create_depth(0, 0, -9999, fader);
				fade1.bounce = true;
				fade1.fadeOverFrames = 20;
				fade1.startingOpacity = 0;
				fade1.targetOpacity = 1;
				fade1.executeOnFirstBounce = function() {
	...
	if (z_pressed) {
		agreedToTermsOfDeathAndDespair = true;
	}
}